...
\&{else}\1\5
${}\\{sign}\K{-}\T{1.0}{}$;\SHC{ for ``$-$'' mover }\SHC{ \PB{\&{double} %
\\{temp}${}\K(\\{zp}/\\{tanh}(\.{Y0})+\\{sign}*\|t)*\\{sinh}(\|Y)-\|z*\\{cosh}(%
\|Y);$} }\2\7
\&{double} \\{temp}${}\K(\\{sign}*\|t)*\\{sinh}(\|Y)-\|z*\\{cosh}(\|Y){}$;\SHC{
Mo Yujun method }\SHC{ \PB{\\{temp}} is the second term in denominator }\7
...
${}\\{yp}\K\\{xpperp}*\\{sin}(\\{phip}){}$;\SHC{ \PB{\&{double} \\{temp}${}\K(%
\\{zp}/\\{tanh}(\.{Y0})+\\{sign}*\|t)*\\{sinh}(\.{Y0})-\|z*\\{cosh}(\.{Y0});$}
}\7
\&{double} \\{temp}${}\K(\\{sign}*\|t)*\\{sinh}(\.{Y0})-\|z*\\{cosh}(%
\.{Y0}){}$;\SHC{ Mo Yujun method }\SHC{ \PB{\\{temp}} is the second term in
denominator }\7
${}\\{denominator}\K\\{pow}(\\{Sq}(\\{xp}-\|x)+\\{Sq}(\\{yp}-\|y)+\\{Sq}(%
\\{temp}),\39\T{1.5});{}$\6
