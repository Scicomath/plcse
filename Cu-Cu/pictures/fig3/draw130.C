...
// to define pad location
 ...
 Double_t x1[n],y1[n];                     
  ifstream in1;                                
  in1.open("a++_130_lambda0.3R.dat");                     
  for(Int_t i=0;i<n;i++){                      
   in1 >>x1[i]>>y1[i];                   
  }                                            
  in1.close();
...
/*
  ...
  */
  ...
  tex = new TLatex(6.,0.0015,"#lambda = 0.3");
  tex->SetLineWidth(2);tex->SetTextSize(0.07);  tex->SetTextFont(12);
  tex->Draw();

  tex = new TLatex(0.80,0.0015,"#sqrt{s_{NN}} = 130 GeV");
  tex->SetLineWidth(2);tex->SetTextSize(0.07);  tex->SetTextFont(12);
  tex->Draw();
...
*/
   c1->cd();
   c1->SaveAs("130.eps");   //save and produce a EPS file
     }
