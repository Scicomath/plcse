...
// to define pad location
 ...
 Double_t x1[n],y1[n];                     
  ifstream in1;                                
  in1.open("a++_200_lambda0.3R.dat");                     
  for(Int_t i=0;i<n;i++){                      
   in1 >>x1[i]>>y1[i];                   
  }                                            
  in1.close();
  ...
  Double_t x2[n],y2[n];                     
  ifstream in2;                                
  in2.open("a+-_200_lambda0.3R.dat");                     
  for(Int_t i=0;i<n;i++){                      
   in2 >>x2[i]>>y2[i];                   
 }                                            
  in2.close(); 
...
/*
  ...
  */
  ...
  tex = new TLatex(0.80,0.00075,"#sqrt{s_{NN}} = 200 GeV");
  tex->SetLineWidth(2);tex->SetTextSize(0.07);  tex->SetTextFont(12);
  tex->Draw();
...
*/
   c1->cd();
   c1->SaveAs("200.eps");   //save and produce a EPS file
     }
