	...
	DOUBLE PRECISION F,S,X,TAU,ELAB,Xmass,ELAB2,ELAB1
        DOUBLE PRECISION y0,b0,RR,bb,Emin,Emax,CMS
	DOUBLE PRECISION tmin,tmax,yy,SS,ssb
	DATA JS/4,4,4,4/
        open(11,file='Bj_p22_b8fm_200_tau.dat',status='unknown')
	N=4
	Xmass=0.939
	bb=8.0
	RR=7.0 
	...
	do ii=0,200
	  tau=tmin+((tmax-tmin)/200.)*float(ii)
	  SS=ssb*(tmin/tau)*exp((-1.0/54.0)*(tau*tau-tmin*tmin))
	  WRITE(11,*) tau,SS
	enddo
