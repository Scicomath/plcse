        EXTERNAL FS,F
	DIMENSION JS(3),X(3),bbx(201),eby(201)
	...
	DATA JS/4,4,4/
        open(111,file='delta+-_2760_lambda0.1R-bb1.dat',
     *      status='unknown')
        open(11,file='eB_2760_bb.dat',status='old') 
        open(112,file='delta+-_2760_lambda0.2R-bb1.dat',
     *      status='unknown')
        open(113,file='delta+-_2760_lambda0.3R-bb1.dat',
     *      status='unknown')
	...
	do ii=1,201
	  read(11,*) bbx(ii),eby(ii)
          bb=bbx(ii)*8.0

	  lambda=0.1*RR
          CALL FGAUS(N,JS,X,FS,F,S,TAU,y0,RR,bb,lambda)
           S1=(eby(ii)/(197.5*197.5))**2*S
	   WRITE(111,*) bb,S1

	  lambda=0.2*RR
	  CALL FGAUS(N,JS,X,FS,F,S,TAU,y0,RR,bb,lambda)
           S2=(eby(ii)/(197.5*197.5))**2*S
	   WRITE(112,*) bb,S2

	  lambda=0.3*RR
	  CALL FGAUS(N,JS,X,FS,F,S,TAU,y0,RR,bb,lambda)
           S3=(eby(ii)/(197.5*197.5))**2*S
	   WRITE(113,*) bb,S3
	  enddo
...
**************************************************************************
	...
	ataui=tau
        exp11=exp(-(1.0/lambda)*abs(sqrt(RR*RR-(x(1)
     *      -0.5*bb)**2)-x(2)))
	exp22=exp(-(1.0/lambda)*abs(sqrt(RR*RR-(x(1)
     *      -0.5*bb)**2)+x(2)))
     ...
     *       (x(3)*x(3)-ataui*ataui))
         ...
         f=-4.0*10.0*(25.0/81.0)*aksi*atau
