	...
	DOUBLE PRECISION F,S,X,TAU,ELAB,Xmass,ELAB2,ELAB1
       DOUBLE PRECISION y0,b0,RR,bb,bmin,bmax,AA1
	DOUBLE PRECISION yy,Emin,Emax,CMS
	DATA JS/4,4,4/
        open(22,file='eB_s22_b4fm_CMS.dat',status='unknown')
        open(222,file='eB_s22_b8fm_CMS.dat',status='unknown')
	N=3
	Xmass=0.939
	RR=8.0 
	  Emin=30.0
	...
	bb=4.0
	do ii=0,10000
	  ELAB=Emin+((Emax-Emin)/10000.)*float(ii)
	   y0=yy(ELAB)
 	 tau=2.0*RR*exp(-y0) 
	   CMS=2.0*ELAB
